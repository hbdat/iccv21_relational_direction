...
class HICODet_pad_Dataset(Dataset):
    def __init__(self, partition, content, label_type = 'interaction'):
        super(HICODet_pad_Dataset, self).__init__() 
        ...
        self.samples = [file_name.split('.')[0] for file_name in self.samples["file"].values]
        self.feature_dir = NFS_path+'data/hico_20160224_det/features_pad/'
        
        self.img_sizes = {}
        
        if partition == 'test':
            self.data = sio.loadmat('./data/hico_20160224_det/anno_bbox.mat') 
            self.bbox_anno = self.data['bbox_test'][0]
            for item in self.bbox_anno:
                file_name = item[0][0].split('.')[0]
                self.img_sizes[file_name] =  np.array([item[1][0,0][0][0,0].astype(np.float),item[1][0,0][1][0,0].astype(np.float)]) #width, height
    ...
    def __getitem__(self, idx):
        ...
        feature_file = image_file+".pkl.gz"
...
#            label[self.i2o[pos_idx]] = 1
        if self.partition == 'test':
            size = self.img_sizes[image_file]
            return image_file,feature,label,size
        else:
